...
<!-- latest clothes -->
<?php  include 'classes/no_woman_product_slide.php' ; ?>


<div class="left" > 

<h1>Clothes for Womans</h1>
...
FROM products as p , images_tbl as i where p.id = i.product_id and (SUBSTRING_INDEX(SUBSTRING_INDEX(p.gender, ',', 1), ',', -1) = 'woman' or SUBSTRING_INDEX(SUBSTRING_INDEX(p.gender, ',', 2), ',', -1) = 'woman' or SUBSTRING_INDEX(SUBSTRING_INDEX(p.gender, ',', 3), ',', -1) = 'woman' or SUBSTRING_INDEX(SUBSTRING_INDEX(p.gender, ',', 4), ',', -1) = 'woman' OR SUBSTRING_INDEX(SUBSTRING_INDEX(p.gender, ',', 5), ',', -1) = 'woman' OR SUBSTRING_INDEX(SUBSTRING_INDEX(p.gender, ',', 6), ',', -1) = 'woman') ORDER by id DESC ");
